...
const DepartingRepoPeriod =()=>{
    ...
    const [data, error, loading] = useFetch({
      ...
      useEffect(()=>{
        ...
        if(datas.Departure_Date_Actual.substring(0,datas.Departure_Date_Actual.length-7) >= `${dataSearchStrt}:` && datas.Departure_Date_Actual.substring(0,datas.Departure_Date_Actual.length-7) <= `${dateSearchEnd}:` )
        {
            setDataNew(current => [...current, datas]);
            console.log(`hwa da el date ${dataNew}`)
...
export default DepartingRepoPeriod;
